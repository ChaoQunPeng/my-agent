<template>
  <a-drawer :open="visible" :title="title" placement="right" :width="1200" @close="onClose">
...
defineOptions({
  name: 'KnowledgeBaseDetailDrawer'
});
...
// 监听知识库ID变化，可以在这里加载不同的知识库数据
watch(
  () => props.knowledgeBaseId,
  newId => {
    if (newId) {
      // 实际项目中这里会根据ID加载对应的详情数据
      console.log('Loading knowledge base detail for ID:', newId);
    }
  },
  { immediate: true }
);
