  ...
  <a-card :title="t('account.settings.account-setting')" :bordered="false">
    <a-list item-layout="horizontal" :data-source="data">
      <template #renderItem="{ item }">
        <a-list-item>
          <a-list-item-meta :description="t('account.settings.account.not.bind')">
            <template #title>
              <a href="https://www.antdv.com/">{{ item.title }}</a>
            </template>
            <template #avatar>
              <TaobaoOutlined
                v-if="item.avatar === 'TaobaoOutlined'"
                style="color: #ff4000"
                class="account-setting-avatar"
              />
              <AlipayOutlined
                v-if="item.avatar === 'AlipayOutlined'"
                style="color: #2eabff"
                class="account-setting-avatar"
              />
              <DingdingOutlined
                v-if="item.avatar === 'DingdingOutlined'"
                style="color: #fff; background-color: #2eabff"
                class="account-setting-avatar"
              />
            </template>
          </a-list-item-meta>
